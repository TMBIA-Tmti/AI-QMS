...
4. 區分三種結果（根據 ISO 13485 稽核實務）：
   - 充分 (found=true, is_inadequate=false)：文件明確描述「如何執行」而非僅「提及」該要求。
   - 不充分 (found=true, is_inadequate=true)：文件提到此要求但缺乏具體程序、責任人或可量測標準。
   - 缺失 (found=false)：文件中完全找不到相關內容。
5. 語言符合性 ≠ 法規正確性：文件使用相似術語不等於實質涵蓋法規要求，需確認實質內容。
